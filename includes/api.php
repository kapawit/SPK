...
function alternatif() {
    global $conn;
    $q = $conn->prepare("SELECT * FROM alternatif WHERE nama NOT IN ( SELECT namasiswa AS nama FROM jurusan )");
    $q->execute();
    @$data = $q->fetchAll();
    if ($data) return $data;
    else return array();
...
function data_siswa() {
    global $conn;
    $q = $conn->prepare("SELECT * FROM siswa");
    $q->execute();
    return @$q->fetchAll();
}
function hasil() {
    global $conn;
    $q = $conn->prepare("SELECT * FROM hasil_saw");
    $q->execute();
    return @$q->fetchAll();
}
function jurusan() {
